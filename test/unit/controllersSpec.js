  ...
  describe('flickrAppControllers', function(){
    ...
    describe('PhotoDetailCtrl', function(){
      ...
      beforeEach(inject(function(_$httpBackend_, $rootScope, $routeParams, $controller){
        $httpBackend = _$httpBackend_;
        $httpBackend.expectJSONP(feedUrl).respond(feedData);
        $routeParams.photo_id = 0;
        scope = $rootScope.$new();
        ctrl = $controller('PhotoDetailCtrl', {$scope: scope});
      }));

      it("should create a 'photo' model that represents the photo selected from feed", function(){
        $httpBackend.flush();
        expect(scope.photo).toEqualData(feedData.items[0]);
      });

      it("should create a 'photoTags' model that is an array of all the photo tags", function(){
        $httpBackend.flush();
        expect(scope.photoTags).toEqualData([
          'christmas', 'food', 'denmark', 'dish', 'sony', 'vegetable', 'sugar', 'caramel', 'potato', 'danish', 'glazed', '1650mm', 'a6000'
        ]);
      });
